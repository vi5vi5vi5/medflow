...
Справочник по HTTP-серверу MedFlow: REST-эндпоинты + раздача статического веб-интерфейса. Источник правды — `api/ApiServer.cpp` и сервисы из `services/`.

- **База API:** `http://localhost:8420/api`
- **Слушает:** `0.0.0.0:8420` (порт захардкожен константой `kPort` в `MedFlow_BY_LLl-23.cpp`)
...
- **Хранилище:** SQLite-файл `backups/medflow.db`. Схема накатывается при старте, данные между запусками сохраняются. JSON-бэкап на старте **не** загружается — восстановление только явно, через `POST /api/backup/load`. При штатном завершении (`SIGINT`/`SIGTERM`) сервер пишет снапшот `backups/bck_<timestamp>.bck`.
...
> Имена полей в JSON именно такие, как ниже (часть в `snake_case`, часть в `camelCase` — так исторически сложилось в моделях). Поля enum'ов сериализуются как целые числа.
...
Тело: `{"path":"..."}`. Если `path` **не задан** — грузится самый свежий `.bck` из `backups/` (имена с ISO-таймстампом, поэтому «последний по имени» = самый новый). Если бэкапов нет — `404`.
```bash
...
### List / Download
`GET /api/backup/list` возвращает простую HTML-страницу (свежие — сверху) со ссылками вида `/api/backup/download?file=<имя>`. Скачивание защищено от path-traversal: имя файла — только базовое, без `/`, `\` и `..`, иначе `400`.
```bash
...
Помимо API сервер раздаёт статический фронтенд из каталога `web/` (ищется как `./web`, затем `../web`, затем `../../web` относительно бинаря). Подробнее — `web/README.md`.
...
Порт — `8420`. Рабочие данные (SQLite-БД и `.bck`-снапшоты) лежат в каталоге `backups/` рядом с бинарём; в Docker этот каталог имеет смысл монтировать наружу для персистентности.

### Локально (Windows, MSVC, x64)
Собрать решение в Visual Studio и запустить бинарь (исходный проект — `MedFlow_BY_LLl-23.cpp`). При старте создаётся `backups/medflow.db`. Health-чек: `http://localhost:8420/api/health`.
...
```
Контейнер слушает `8420`, корректно реагирует на `docker stop` (`SIGTERM` → graceful-остановка + снапшот на выходе). Статика `web/` копируется в образ рядом с бинарём.
