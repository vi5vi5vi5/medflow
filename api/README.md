...
```
Контейнер слушает `8420`, корректно реагирует на `docker stop` (`SIGTERM` → graceful-остановка + снапшот на выходе). Статика `web/` копируется в образ рядом с бинарём. Полный гайд по деплою — [`../tools/README.md`](../tools/README.md).
